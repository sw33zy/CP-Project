...
--prop_avg :: Fractional a => [a] -> Property
...
outExpAr (Bin b e1 e2) = i2 ( i2 (i1 (b,(e1,e2))))
outExpAr (Un u e1) =   i2 ( i2 (i2 (u,e1)))
\end{code}
...
\begin{code}
g_eval_exp a = either (const a)  (either id  (either auxBin  auxUn)) where
auxBin (Sum,(a,b)) = a + b
auxBin (Product,(a,b)) = a * b
auxUn (Negate, a) = -a
auxUn (E, a) = expd a
\end{code}
...
&
    |1 + (A + (BinOp >< (ExpAr A >< ExpAr A) + UnOp >< ExpAr A))|
           \ar[d]^{|recExpAr (cata (g_eval_exp a))|}
\\
     |A|
&
...
\begin{code}
clean (Bin Product (N 0.0) _) = i2 (i1 0.0)
clean (Bin Product _ (N 0.0)) = i2 (i1 0.0)
clean a = outExpAr a
\end{code}
...
\begin{code}
sd_gen :: Floating a =>
    Either () (Either a (Either (BinOp, 
    ((ExpAr a, ExpAr a), (ExpAr a, ExpAr a))) (UnOp, (ExpAr a, ExpAr a)))) -> (ExpAr a, ExpAr a)
sd_gen = either sd_deriv_X  (either sd_deriv_N  (either sd_deriv_Bin  sd_deriv_Un)) where
sd_deriv_X () = (X,(N 1.0))
sd_deriv_N a = ((N a),(N 0.0))
sd_deriv_Bin (Sum,((a,da), (b,db))) = ((Bin Sum a b),(Bin Sum da db))
sd_deriv_Bin (Product,((a,da), (b,db))) = ((Bin Product a b),
  (Bin Sum (Bin Product a db) (Bin Product da b)))
sd_deriv_Un (Negate, (a,da)) = ((Un Negate a), (Un Negate da))
sd_deriv_Un (E, (a,da)) = ((Un E a), (Bin Product (Un E a) da))
\end{code}
\subsubsection*{Diagrama do Catamorfismo}
\begin{eqnarray*}
\xymatrix@@C=2cm{
...
&
    |1 + (A + (BinOp >< (ExpAr A >< ExpAr A) + UnOp >< ExpAr A))|
           \ar[d]^{|recExpAr (cata (sd_gen))|}
\\
     |(ExpAr A)quadrado|
&
     |1 + (A + (BinOp >< ((ExpAr A)quadrado >< (ExpAr A)quadrado) + UnOp >< (ExpAr A)quadrado)|
           \ar[l]^-{|sd_gen|}
}
\end{eqnarray*}
...
resultante da derivação.
\begin{code}
ad_gen v = either (ad_deriv_X v)  (either ad_deriv_N  (either ad_deriv_Bin  ad_deriv_Un)) where
ad_deriv_X v () = (v,1.0)
ad_deriv_N a = (a,0.0)
ad_deriv_Bin (Sum,((a,da), (b,db))) = (a+b,da + db)
ad_deriv_Bin (Product,((a,da), (b,db))) = (a*b, (a*db)+(da*b))
ad_deriv_Un (Negate, (a,da)) = (-a, -da)
ad_deriv_Un (E, (a,da)) = (expd a, da * expd a)
\end{code}
\subsubsection*{Diagrama do Catamorfismo}
\begin{eqnarray*}
\xymatrix@@C=2cm{
    |ExpAr A|
           \ar[d]_-{|cata (ad_gen a)|}
           \ar[r]_-{|outExpAr|}
&
    |1 + (A + (BinOp >< (ExpAr A >< ExpAr A) + UnOp >< ExpAr A))|
           \ar[d]^{|recExpAr (cata (ad_gen a))|}
\\
     |A quadrado|
&
...
\end{itemize}
\begin{code}
loop (n,(ca,next)) = (n+1,(next, (div (next*(4*n+2)) (n+2))))
\end{code}
...
calcLine = cataList h where
   h = either aux1 aux2 where
   aux1 l = const nil
   aux2 :: (Rational, NPoint -> OverTime NPoint) -> (NPoint -> OverTime NPoint)
...
&
    |1 + (Rational >< NPoint)|
           \ar[d]^{|id + (id >< cata (h))|}
\\
     |(NPoint setaDireita OverTime NPoint)|
&
     |1 + (Rational >< (NPoint setaDireita OverTime NPoint))|
           \ar[l]^-{|h|}
}
\end{eqnarray*}
...
deCasteljau = hyloAlgForm alg coalg where
  ...
  alg = either const auxdC where
    auxdC (op1,op2) = \pt -> (calcLine (op1 pt) (op2 pt)) pt
...
\\
     |NPoint estrela|    
           ...
           \ar[r]^-{|coalg|}
&
     |NPoint + (NPoint estrela >< NPoint estrela)|
           \ar[u]^{|id + (ana (coalg) >< ana (coalg))|}
}
\end{eqnarray*}
...
&
    |NPoint + (LTree NPoint >< LTree NPoint)|
           \ar[d]^{|id + (cata (alg) >< cata (alg))|}
\\
     |OverTime NPoint|
&
     |id + ((OverTime NPoint) >< (OverTime NPoint))|
           \ar[l]^-{|alg|}
}
\end{eqnarray*}
...
\begin{code}
avg_aux = cataList (either (const (0,0)) (split calc_avg ((1+) . p2 . p2))) where
calc_avg (a,(av,len)) = (a +(av * len))/(len+1)
\end{code}
...
avgLTree = p1.cataLTree gene where
  gene = either (split id (const 1)) (split calc_avg_tree calc_len_tree)  where
    calc_avg_tree ((al,ll),(ar,lr)) = ((al*ll)+(ar*lr))/(ll+lr)
    calc_len_tree ((_,ll),(_,lr)) = ll + lr
\end{code}
...
\xymatrix@@C=2cm{
    |LTree Double|
           ...
           \ar[r]_-{|outLTree|}
&
    |Double + (LTree Double >< LTree Double)|
